...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    infoln "Creating Distributors Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-distributors.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
    fi
    infoln "Creating Retailers Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-retailers.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
    ...
    infoln "Creating Transporters Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-transporters.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
  ...
  while :
    do
      ...
      if [ ! -f "organizations/fabric-ca/wholesaler/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
      if [ ! -f "organizations/fabric-ca/distributors/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
      if [ ! -f "organizations/fabric-ca/retailers/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
      if [ ! -f "organizations/fabric-ca/transporters/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
  ...
  infoln "Generating CCP files for Farmer and Wholesaler ,Distributors,Retailers,Transporters"
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    docker run --rm -v $(pwd):/data busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/distributors/msp organizations/fabric-ca/distributors/tls-cert.pem organizations/fabric-ca/distributors/ca-cert.pem organizations/fabric-ca/distributors/IssuerPublicKey organizations/fabric-ca/distributors/IssuerRevocationPublicKey organizations/fabric-ca/distributors/fabric-ca-server.db'
    docker run --rm -v $(pwd):/data busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/retailers/msp organizations/fabric-ca/retailers/tls-cert.pem organizations/fabric-ca/retailers/ca-cert.pem organizations/fabric-ca/retailers/IssuerPublicKey organizations/fabric-ca/retailers/IssuerRevocationPublicKey organizations/fabric-ca/retailers/fabric-ca-server.db'
    docker run --rm -v $(pwd):/data busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/transporters/msp organizations/fabric-ca/transporters/tls-cert.pem organizations/fabric-ca/transporters/ca-cert.pem organizations/fabric-ca/transporters/IssuerPublicKey organizations/fabric-ca/transporters/IssuerRevocationPublicKey organizations/fabric-ca/transporters/fabric-ca-server.db'
